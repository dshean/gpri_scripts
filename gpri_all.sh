...
#Note: gpri usr/bin/head is HEAD
mk_int_2d slc_tab itab $(ls $mlidir/*l.mli | /usr/bin/head -1) $mlidir diff_${rlks} $rlks $azlks 3 0 0 1 -s 0.7 -e 0.35

#Filter w/ adaptive bandpass filter
mk_adf_2d slc_tab itab $(ls $mlidir/*l.mli | /usr/bin/head -1) diff_${rlks} 5 0.4 32 4 -s 0.7 -e 0.35
...
mk_unw_2d slc_tab itab $(ls $mlidir/*l.mli | /usr/bin/head -1) diff_${rlks} $c_thr $i_thr 1 1 1 1 $r_ref $a_ref 1 -d diff_tab_${rlks} -s 0.7 -e 0.35 -p 1.0
