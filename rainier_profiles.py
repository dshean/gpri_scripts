...
a = np.genfromtxt(csv_fn, delimiter=',', missing_values=0, usemask=True, names=True)
names = np.array(a.dtype.names).reshape(6,2)
...
win = 2*h/avg_dx
y_ma = np.ma.masked_equal(a[y], 0)
y_slope_smooth = np.ma.diff(malib.nanfill(y_ma, smooth2, win))
